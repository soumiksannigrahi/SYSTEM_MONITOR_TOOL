...
int main() {
    ...
    int highlight = 0;
    int interval_s = 2;
    enum SortMode {BY_CPU, BY_MEM, BY_PID, BY_NAME};
    ...
    while (true) {
        ...
        auto cmp = [&](const ProcInfo &a, const ProcInfo &b) -> bool {
            bool result;
            switch (sortmode) {
                case BY_CPU:  result = a.cpu_percent > b.cpu_percent; break;
                case BY_MEM:  result = a.mem_percent > b.mem_percent; break;
                case BY_NAME: result = a.cmd < b.cmd; break;
        ...
        mvprintw(0, 0, "SimpleSysMon  (q=quit c=CPU m=MEM p=PID n=NAME r=reverse k=kill +/- interval=%ds)", interval_s);
        const char *sort_str[] = {"CPU%", "MEM%", "PID", "NAME"};
        mvprintw(0, 75, "Sort: %s %s", sort_str[sortmode], reverse_sort ? "ASC" : "DESC");
        ...
        if (ch != ERR) {
            ...
            if (ch == 'n') sortmode = BY_NAME;
            if (ch == 'r') reverse_sort = !reverse_sort;
            if (ch == '+') interval_s = std::min(10, interval_s + 1);
            if (ch == '-') interval_s = std::max(1, interval_s - 1);
            if (ch == 'k' && highlight >= 0 && highlight < (int)procs.size()) {
                int pid = procs[highlight].pid;
                mvprintw(2, 0, "Kill PID %d? (y/n): ", pid);
        ...
        // Non-blocking refresh: poll input during wait (Day 5)
        for (int t = 0; t < interval_s * 10; ++t) {
            int c = getch();
            if (c != ERR) { ungetch(c); break; }
